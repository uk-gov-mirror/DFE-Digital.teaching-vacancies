...
RSpec.describe DfeSignIn::API do
  # Reuses the existing two-page fixtures so this spec is a drop-in replacement for the
  # current page-object based one, not a new fixture format to review.
  let(:first_page) { File.read(Rails.root.join("spec/fixtures/dfe_sign_in_service_users_response_page_1.json")) }
  let(:second_page) { File.read(Rails.root.join("spec/fixtures/dfe_sign_in_service_users_response_page_2.json")) }
  ...
  describe ".users" do
    ...
    context "when the source has no users at all" do
      let(:empty) { File.read(Rails.root.join("spec/fixtures/dfe_sign_in_service_users_empty_response.json")) }

      before do
        stub_request(:get, "#{ENV.fetch('DFE_SIGN_IN_URL', nil)}/users?page=1&pageSize=275")
          .to_return(body: empty, status: 200)
      end
    ...
    it "authenticates each request with a freshly signed bearer token" do
      ...
      freeze_time do
        authenticated_request = stub_request(:get, "#{ENV.fetch('DFE_SIGN_IN_URL', nil)}/users?page=1&pageSize=275")
          ...
          .to_return(body: first_page, status: 200)
        # first_page's fixture reports numberOfPages: 2, so the enumerator goes on to
  ...
  # .approvers should be the same case as users, just a different endpoint and fixture. The two are tested side by side to make it obvious that the only difference is the endpoint, not the paging logic.
  describe ".approvers" do
    let(:first_approvers_page) do
      File.read(Rails.root.join("spec/fixtures/dfe_sign_in_service_approvers_response_page_1.json"))
    end
    let(:second_approvers_page) do
      File.read(Rails.root.join("spec/fixtures/dfe_sign_in_service_approvers_response_page_2.json"))
    end
    ...
    it "requests the approvers endpoint with the approvers page size" do
      stub = stub_request(:get, "#{ENV.fetch('DFE_SIGN_IN_URL', nil)}/users/approvers?page=1&pageSize=275")
        .to_return(body: first_approvers_page, status: 200)
      # first_approvers_page's fixture reports numberOfPages: 2, so the enumerator goes on
      # to request page 2 too.
      stub_request(:get, "#{ENV.fetch('DFE_SIGN_IN_URL', nil)}/users/approvers?page=2&pageSize=275")
        .to_return(body: second_approvers_page, status: 200)
